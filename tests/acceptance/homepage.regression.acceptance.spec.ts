// DGF-413: Node-only regression tests for CMS homepage logic.
...
test.describe('DGF-413 — Homepage CMS Regression', () => {
  let originalHomepageData: any;
  ...
  test.afterAll(async () => {
    if (originalHomepageData) {
      ...
      console.log('\n[DGF-413] Restored original homepage data.');
    }
  });

  test('homepage.save writes hero content to Firestore', async () => {
    const testMarker = `DGF-413 regression test ${Date.now()}`;
    const testPayload = {
      ...originalHomepageData,
      hero: {
        ...originalHomepageData.hero,
        slides: [{
          ...originalHomepageData.hero.slides[0],
          eyebrow: testMarker,
        }],
      },
    };
  ...
  test('homepage.read returns data consistent with expected schema', async () => {
