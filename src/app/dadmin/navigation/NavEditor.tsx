...
import { NavigationSchema, NavLinkSchema, type Navigation, type NavLink } from "@/lib/schemas";
...
function NavItemsList({ title, namePrefix, pages }: { title: string; namePrefix: 'header' | 'footer.columns.0.links'; pages: PageInfo[] }) {
          ...
          <SortableContext items={fields.map(f => f.fieldId)} strategy={verticalListSortingStrategy}>
            <div className="space-y-2">
              {fields.map((field, index) => (
                <SortableItem key={field.fieldId} id={field.fieldId} index={index} control={control} remove={() => setDeleteIndex(index)} namePrefix={namePrefix} pages={pages} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
        <div className="flex justify-between items-center pt-4">
          <Button type="button" variant="outline" size="sm" onClick={() => append({ link: { type: 'internal', internalRef: '', label: '' } })}>
...
export default function NavEditor({ value, onChange, onSave, saving, pages }: {
  value: Navigation;
  onChange: (v: Navigation) => void;
  onSave: (v: Navigation) => Promise<void>;
  saving: boolean;
  pages: PageInfo[];
}) {
  ...
  const onSubmit = methods.handleSubmit(async (data) => {
    await onSave(data);
    methods.reset(data); // Re-sync to clear dirty state
  });
      ...
      <Form {...methods}>
        <form onSubmit={onSubmit} className="space-y-8">
          ...
          <div className="sticky bottom-0 bg-slate-50/90 py-4 dark:bg-slate-900/90 border-t -mx-6 px-6">
            <Button type="submit" disabled={saving || !methods.formState.isDirty}>
