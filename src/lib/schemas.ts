...
export const zRichText: z.ZodType<RichTextContent[]> = z.array(
  z.union([
...
export const zAboutPage = zPage.extend({
    content: z.object({
        body: zRichText,
    }).default({ body: [] }),
});

export const zServicesPage = zPage.extend({
    content: z.object({
...
export const zCasesIndexPage = zPage.extend({
    content: z.object({}).default({})
});

export const zContactPage = zPage.extend({
    content: z.object({}).default({})
});
...
export function parseCase(input: unknown): CaseDoc {
  const parsed = zCase.safeParse(input);
  if (!parsed.success) {
    // In production, you could log parsed.error to your logging service
    // For now, we return a safe, default structure.
    return {
      slug: "unknown-case",
      title: "Untitled Case",
      seo: {},
